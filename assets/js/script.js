class TrafficLight {
    ...
    switchToRed() {
        ...
        this.red.style.boxShadow ='0px 0px 15px rgba(255, 0, 0, 0.7)'
        this.switchToGray([this.yellow, this.green])
    }

    switchToYellow() {
        this.yellow.style.background = 'yellow'
        this.yellow.style.boxShadow = '0px 0px 15px rgba(255, 255, 0, 0.7)'
        this.switchToGray([this.red, this.green])
    }

    switchToGreen() {
        this.green.style.background = 'green';
        this.green.style.boxShadow = '0px 0px 55px rgba(0, 255, 0, 0.7)'
...
switchBtn.addEventListener('click', (event) => {
    if (isRunning) {
        ...
        tf2.switchAllToGray()
        tf3.switchAllToGray()
        switchBtnText.innerText = 'On'
    } else {
        switchBtnText.innerText = 'Off'
        initLights()
    }
    isRunning = !isRunning
})